...
# The following functions are from nibabel. 
# Credit due to authors of nibabel: https://nipy.org/nibabel/
...
# The following functions are from DiCIPHR lab. 
# author: @parkerwi
...
def reorient_bvec(bvec, old_orientation, new_orientation): 
    ...
    ----------
    bvec : numpy.ndarray
        An (3,N) bvec array as read from .bvec file 
    old_orientation : str
    ...
    if not _is_valid_orientation(old_orientation):
        raise ValueError('Not a valid orientation: {}'.format(old_orientation))
    if not _is_valid_orientation(new_orientation):
        raise ValueError('Not a valid orientation: {}'.format(new_orientation))
    orig_orient_code = tuple(s.upper() for s in old_orientation)
    new_orient_code = tuple(s.upper() for s in new_orientation)
    orig_orient_ornt = axcodes2ornt(orig_orient_code)
    new_orient_ornt = axcodes2ornt(new_orient_code)
    trans_ornt = ornt_transform(orig_orient_ornt,
                                                  new_orient_ornt)
    trans_affine = inv_ornt_aff(trans_ornt, (128,128,80))  #dummy nifti shape
    trans_matrix = trans_affine[0:3,0:3]  #discard the translation portion 
    bvec = np.array(bvec).transpose()   # (N, 3)
    bvec_out = np.dot(bvec, trans_matrix) 
    bvec_out = bvec_out.transpose()   # (3, N) 
    return bvec_out