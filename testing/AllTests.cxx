...
int main(int argc, char** argv)
{
  ...
  if (parser.isPresent("geodesic"))
  {
    ...
    if (cbica::directoryExists(dataDir + "CaPTk_SampleData/Geodesic"))
    {
      dataDir = dataDir + "Geodesic/";
      using ImageType = itk::Image< short, 3 >;
      auto inputImage = cbica::ReadImage< ImageType >(dataDir + "input_flair.nii.gz");
      auto mask = cbica::ReadImage< ImageType >(dataDir + "drawing.nii.gz");
      auto outputToCheck = cbica::ReadImage< ImageType >(dataDir + "input_flair_geos.nii.gz");
  ...
  {
    std::string dataDir = DATA_DIR;
    if (cbica::directoryExists(dataDir + ""CaPTk_SampleData/EGFR"))
    {
      dataDir = dataDir + "EGFR/";
      ...
      auto perfusionImage = cbica::ReadImage< ImageTypePerfusion >(dataDir + "AAAC0_perf_pp.nii.gz");
      auto maskImage = cbica::ReadImage< ImageType >(dataDir + "drawing.nii.gz");
      typedef itk::ImageRegionIteratorWithIndex< ImageType > IteratorType;
      ...
      auto EGFRStatusParams = egfrEstimator.PredictEGFRStatus< ImageType, ImageTypePerfusion >(perfusionImage, Perfusion_Registered, nearIndices, farIndices, IMAGE_NIFTI);
  ...
  if (parser.isPresent("recurrence"))
  {
    ...
    if (cbica::directoryExists(dataDir + ""CaPTk_SampleData/Recurrence"))
    {
      ...
      imagesWithModalities[IMAGE_TYPE_T1] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_T1CE] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_T2] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_T2FLAIR] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_DTI + AD] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_DTI + AX] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_DTI + B0] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_DTI + FA] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_DTI + TR] = ImageType::New();
      imagesWithModalities[IMAGE_TYPE_DTI + TR + 1] = ImageType::New();
      ...
      for (size_t i = 0; i < filesForSub.size(); i++)
      {
        ...
        case IMAGE_TYPE_T1:
        {
          imagesWithModalities[IMAGE_TYPE_T1] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          break;
        }
        case IMAGE_TYPE_T1CE:
        {
          imagesWithModalities[IMAGE_TYPE_T1CE] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          break;
        }
        case IMAGE_TYPE_T2:
        {
          imagesWithModalities[IMAGE_TYPE_T2] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          break;
        }
        case IMAGE_TYPE_T2FLAIR:
        {
          imagesWithModalities[IMAGE_TYPE_T2FLAIR] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          break;
        }
        case IMAGE_TYPE_DTI:
        {
          ...
          {
            imagesWithModalities[IMAGE_TYPE_DTI + AD] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          }
          else if (filesForSub[i].find("AX") != std::string::npos)
          {
            imagesWithModalities[IMAGE_TYPE_DTI + AX] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          }
          else if (filesForSub[i].find("B0") != std::string::npos)
          {
            imagesWithModalities[IMAGE_TYPE_DTI + B0] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          }
          else if (filesForSub[i].find("FA") != std::string::npos)
          {
            imagesWithModalities[IMAGE_TYPE_DTI + FA] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          }
          else if (filesForSub[i].find("TR") != std::string::npos)
          {
            imagesWithModalities[IMAGE_TYPE_DTI + TR] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          }
          break;
        }
        default: // this is for the drawing
        {
          ...
          {
            imagesWithModalities[IMAGE_TYPE_DTI + TR + 1] = cbica::ReadImage<ImageType>(dataDir + filesForSub[i]);
          }
          break;
        }
        }
