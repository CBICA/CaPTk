	...
	const std::string outputdirectory)
{
    ...
    if(result[subjectID]>0)
      std::cout << static_cast<std::string>(onesubject[IMAGE_TYPE_SUDOID]) << ": Mutation detected." << std::endl;
    else
      std::cout << static_cast<std::string>(onesubject[IMAGE_TYPE_SUDOID]) << ": Mutation not detected." << std::endl;
	}
	return EXIT_SUCCESS;
}
