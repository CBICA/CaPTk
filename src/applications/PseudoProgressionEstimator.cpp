...
bool PseudoProgressionEstimator::TrainNewModelOnGivenData(const std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects, const std::string &outputdirectory, bool useConventionalData, bool useDTIData, bool usePerfData, bool useDistData)
{
  ...
  try
  {
    trainOpenCVSVM(PseudoModelSelectedFeatures, outputdirectory + "/" + mPseudoTrainedFile, false, Survival);
    trainOpenCVSVM(RecurrenceModelSelectedFeatures, outputdirectory + "/" + mRecurrenceTrainedFile, false, Survival);
  }
  catch (const std::exception& e1)
  {
...
bool PseudoProgressionEstimator::PseudoProgressionEstimateOnExistingModel(std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects,
...
//  std::cout << "selected features done: size:" << PseudoModelSelectedFeatures.Rows() << " columns: " << PseudoModelSelectedFeatures.Cols() << std::endl;
  try
  {
    ...
    else if (cbica::fileExists(modeldirectory + "/Pseudo_SVM_Model.xml") == true && cbica::fileExists(modeldirectory + "/Recurrence_SVM_Model.xml") == true)
    {
      ...
      {
        std::map<ImageModalityType, std::string> currentsubject = qualifiedsubjects[i];
        myfile << static_cast<std::string>(currentsubject[IMAGE_TYPE_SUDOID]) + "," + std::to_string(result_6[i]) + "," + std::to_string(result_18[i]) + "\n";
      }
    }
    myfile.close();
  }
...
VariableSizeMatrixType PseudoProgressionEstimator::LoadPseudoProgressionFeaturesData(const std::vector<std::map<ImageModalityType, std::string>> &trainingsubjects, std::vector<double> &traininglabels,std::string outputdirectory)
{
  ...
  for (unsigned int sid = 0; sid < trainingsubjects.size(); sid++)
  {
    ...
    std::map<ImageModalityType, std::string> currentsubject = trainingsubjects[sid];
    ...
    auto perfImagePointerNifti = m_obj.Read4DNiftiImage(static_cast<std::string>(currentsubject[IMAGE_TYPE_PERFUSION]));
