...
VectorDouble TrainingModule::InternalCrossValidationResubstitution(VariableSizeMatrixType inputFeatures, std::vector<double> inputLabels, double cValue, double gValue, int kerneltype)
{
  ...
  std::ofstream myfile;
  myfile.open("E:/Predictions.csv");
  for (unsigned int index1 = 0; index1 < predictedLabels.Size(); index1++)
      myfile << std::to_string(predictedLabels[index1]) << "\n";
  myfile.close();
  myfile.open("E:/Distances.csv");
  for (unsigned int index1 = 0; index1 < predictedLabels.Size(); index1++)
    myfile << std::to_string(predictedDistances[index1]) << "\n";
  myfile.close();

  myfile.open("E:/InputFeatures.csv");
  for (unsigned int index1 = 0; index1 < inputFeatures.Rows(); index1++)
    myfile << std::to_string(inputFeatures[index1][0]) << "\n";
  myfile.close();

  myfile.open("E:/InputLabels.csv");
  for (unsigned int index1 = 0; index1 < predictedLabels.Size(); index1++)
    myfile << std::to_string(inputLabels[index1]) << "\n";
  myfile.close();

  svm->save("E:/CurrentModel.xml");
...
bool TrainingModule::Run(const std::string inputFeaturesFile, const std::string inputLabelsFile, const std::string outputdirectory, const int classifiertype, const int foldtype, const int confType, const std::string modeldirectory)
{
  ...
  {
    if (mTrainingSimulator.TrainData2(FeaturesOfAllSubjects, LabelsOfAllSubjects, outputdirectory, classifiertype) == true)
    ...
    for (unsigned int featureNo = 0; featureNo < UnselectedFeatures.size(); featureNo++)
    {
      ...
      if (classifiertype == 2)
      {
        for (double cValue = -5; cValue <= 5; cValue = cValue + 1)
          ...
          {
            VectorDouble result = InternalCrossValidationResubstitution(reducedFeatureSet, traininglabels, pow(2, cValue), pow(2, gValue), classifiertype);
            if (result[3] > bestCV)
            {
              bestC = pow(2, cValue);
              bestG = pow(2, gValue);
      ...
      else
      {
        for (double cValue = -5; cValue <= 5; cValue = cValue + 1)
        {
          VectorDouble result = InternalCrossValidationResubstitution(reducedFeatureSet, traininglabels, pow(2, cValue), 0.01, classifiertype);
          if (result[3] > bestCV)
          {
            bestC = pow(2, cValue);
            bestCV = result[3];
