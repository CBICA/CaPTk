...
public:
  ...
  */
  bool TrainNewModelOnGivenData(const std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects, const std::string &outputdirectory, bool useConventioalData, bool useDTIData, bool usePerfData, bool useDistData);

  VariableSizeMatrixType LoadPseudoProgressionFeaturesData(const std::vector<std::map<ImageModalityType, std::string>> &trainingsubjects, std::vector<double> &traininglabels, std::string outputdirectory);

  /**
  \brief Recurrence Estimation using existing model
  */
  bool PseudoProgressionEstimateOnExistingModel(std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects,
