	...
	const VariableSizeMatrixType &configuration)
{
	...
	while (!imIt.IsAtEnd())
	{
		...
		else if (imIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING)
			necoreIndices.push_back(imIt.GetIndex());
		else if (imIt.Get() == GLISTR_OUTPUT_LABELS::EDEMA)
			edemaIndices.push_back(imIt.GetIndex());
		else if (imIt.Get() == GLISTR_OUTPUT_LABELS::VENT)
	...
	typename ImageType::Pointer  edemaDistanceMap = GetDistanceMapWithLabel<ImageType>(labelImagePointer, GLISTR_OUTPUT_LABELS::EDEMA);
	...
	{
		if (labelIt.Get() == GLISTR_OUTPUT_LABELS::VENT)
