...
public:
  ...
  ~SurvivalPredictor() {};
	...
	*/
	...
	template<class ImageType>
	typename ImageType::Pointer GetDistanceMapWithLabel(const typename ImageType::Pointer &labelImagePointer, const int &label1, const int &label2);
...
template<class ImageType>
typename ImageType::Pointer SurvivalPredictor::GetDistanceMapWithLabel(const typename ImageType::Pointer &labelImagePointer, const int &label1, const int &label2)
{
	...
	while (!interIt.IsAtEnd())
	{
		...
		if (imIt.Get() == label1 || imIt.Get() == label2)
			interIt.Set(VOXEL_STATUS::ON);
		else
			interIt.Set(VOXEL_STATUS::OFF);
		++interIt;
		++imIt;
	}
	typedef itk::Image<unsigned char, 3>  UnsignedCharImageType;
	...
	const VariableSizeMatrixType &configuration)
{
	...
	while (!imIt.IsAtEnd())
	{
		if (imIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR_1 || imIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR_2)
			etumorIndices.push_back(imIt.GetIndex());
		else if (imIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_1 || imIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_2)
			necoreIndices.push_back(imIt.GetIndex());
		else if (imIt.Get() == GLISTR_OUTPUT_LABELS::EDEMA_1 || imIt.Get() == GLISTR_OUTPUT_LABELS::EDEMA_2)
			edemaIndices.push_back(imIt.GetIndex());
		else if (imIt.Get() == GLISTR_OUTPUT_LABELS::VENT_1 || imIt.Get() == GLISTR_OUTPUT_LABELS::VENT_2)
			ventIndices.push_back(imIt.GetIndex());
		else
		{
		}
		if (imIt.Get() > GLISTR_OUTPUT_LABELS::ALL)
			brainIndices.push_back(imIt.GetIndex());
		if (imIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR_1 || imIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR_2 || imIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_1 || imIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_2)
	...
	while (!tumorIt.IsAtEnd())
	{
		if (tumorIt.Get() == VOXEL_STATUS::ON)
	...
	typename ImageType::Pointer  edemaDistanceMap = GetDistanceMapWithLabel<ImageType>(labelImagePointer, GLISTR_OUTPUT_LABELS::EDEMA_1, GLISTR_OUTPUT_LABELS::EDEMA_2);
	...
	{
		if (labelIt.Get() == GLISTR_OUTPUT_LABELS::VENT_1 || labelIt.Get() == GLISTR_OUTPUT_LABELS::VENT_2)
			ventIt.Set(VOXEL_STATUS::ON);
		++labelIt;
		++ventIt;
	}
...
std::vector<typename ImageType::Pointer> SurvivalPredictor::RevisedTumorArea(const typename ImageType::Pointer &labelImagePointer)
{
	...
	while (!tumorIt.IsAtEnd())
	{
		...
		if (imIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_1 || imIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_2)
...
typename ImageType::Pointer SurvivalPredictor::RemoveSmallerComponentsFromTumor(const typename ImageType::Pointer &etumorImage, const typename ImageType::Pointer &ncrImage)
{
	...
	while (!tumorIt.IsAtEnd())
	{
		if (etIt.Get() == VOXEL_STATUS::ON || ncrIt.Get() == VOXEL_STATUS::ON)
			tumorIt.Set(VOXEL_STATUS::ON);
		else
			tumorIt.Set(VOXEL_STATUS::OFF);
...
VectorDouble SurvivalPredictor::GetSpatialLocationFeatures(const typename ImageType::Pointer &labelImagePointer, const typename ImageType::Pointer &jacobtemplateImagePointer)
{
	...
	//template<class ImageType>
	//typename ImageType::Pointer SurvivalPredictor::CalculateSERFeature(const typename ImageType::Pointer &enhancement1, const typename ImageType::Pointer &enhancement2)
	//{
	//	typename ImageType::Pointer localizeImage = ImageType::New();
	...
	//template<class ImageType>
	//typename ImageType::Pointer SurvivalPredictor::CalculatePEFeature(const typename ImageType::Pointer &enhancement1, const typename ImageType::Pointer &enhancement2)
	//{
	//	typename ImageType::Pointer localizeImage = ImageType::New();
	//	localizeImage->CopyInformation(enhancement1);
	//	localizeImage->SetRequestedRegion(enhancement1->GetLargestPossibleRegion());
	...
	//template<class ImageType>
	//typename ImageType::Pointer SurvivalPredictor::CalculateWOSFeature(const typename ImageType::Pointer &enhancement1, const typename ImageType::Pointer &enhancement1, const double &timepoint1, , const double &timepoint2)
	//{
	//	typename ImageType::Pointer localizeImage = ImageType::New();
	//	localizeImage->CopyInformation(enhancement1);
	//	localizeImage->SetRequestedRegion(enhancement1->GetLargestPossibleRegion());
	...
	//template<class ImageType>
	//typename ImageType::Pointer SurvivalPredictor::CalculateKineticFeatures(const typename ImageType::Pointer &enhancement1, const typename ImageType::Pointer &enhancement2, const typename ImageType::Pointer &enhancement3, const double &timepoint1, , const double &timepoint2, const double &timepoint3)
	//{
	...
	//}
	//template<class ImageType>
	//typename ImageType::Pointer SurvivalPredictor::CalculateWISFeature(const typename ImageType::Pointer &enhancement1, const typename ImageType::Pointer &enhancement1, const double &timepoint1, , const double &timepoint2)
	//{
	//}
