  ...
  bool useConventionalData,bool useDTIData, bool usePerfData, bool useDistData)
{
  ...
  if (useDistData)
  {
	...
	while (!TumIt.IsAtEnd())
	{
		if (TumIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR_1 || TumIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR_2 || TumIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_1 || TumIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_2)
			tumorIndices.push_back(TumIt.GetIndex());

		else if (TumIt.Get() == GLISTR_OUTPUT_LABELS::EDEMA_1 || TumIt.Get() == GLISTR_OUTPUT_LABELS::EDEMA_2)
