/**
\file  TrainingModuleLegacyUtils.h

\brief The header file containing the TrainingModuleLegacyUtils class, used to build machine learning models in CaPTK applications that are not updated for newer TrainingModule code.
...
class TrainingModuleLegacyUtils
#ifdef APP_BASE_CAPTK_H
...
template <typename T>
std::vector<size_t> TrainingModuleLegacyUtils::sort_indexes(const std::vector<T> &v)
{
