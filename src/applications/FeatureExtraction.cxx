  ...
  std::string application, std::vector<std::string> image_file_name, std::vector<std::string> modality)
{
  ...
  if (!patient_id.empty() && !image_path.empty())
  {
    ...
    if (features.m_roi.size() != 0)
    {
      for (size_t x = 0; x < features.m_roi.size(); x++)
      {
        imageIterator.GoToBegin();
        while (!imageIterator.IsAtEnd())
        {
          if (imageIterator.Get() == features.m_roi[x])
            break;
          ++imageIterator;
          if (imageIterator.IsAtEnd())
          {
            std::cout << "The provided label doesnot exist in the mask";
            EXIT_FAILURE;
          }
  ...
  if (!multipatient_file.empty())
  {
    ...
    if (!ifile.good())
    {
      std::cout << "Provided file" + multipatient_file + " is not good for reading ";
    }
    else
    {
    ...
    for (int j = 0; j < allRows.size(); j++)
    {
      for (int k = 0; k < allRows[0].size(); k++)
      {
        if (j > 0)
        {
          if (allRows[0][k] == "T1" || allRows[0][k] == "T2"
            || allRows[0][k] == "T1CE" || allRows[0][k] == "FLAIR")
          {
            string path, basename, ext;
            cbica::splitFileName(allRows[j][k], path, basename, ext);
            if (!path.empty())
              image_file_name.push_back(allRows[j][k]);
            else
              modality.push_back(allRows[0][k]);
          }
          if (allRows[0][k] == "ROI")
          {
            maskfilename = fileChecker(allRows[j][k], csvPath);
          }
          ...
          if (allRows[0][k] == "SELECTED_FEATURES")
          {
            selected_features = allRows[j][k];
          }
          if (allRows[0][k] == "PARAM_FILE")
          {
...
std::pair< std::string, std::map< std::string, double > > customFunction(cbica::CmdParser &currentCmdParser, const std::string &inputString,
  const std::string &paramFile)
{
  ...
  {
    stringstream param_stringstream(tempFeatureParam);
    while (getline(param_stringstream, parsed, ','))
    {
      stringstream ss(parsed);
      vector<string> tokens; std::string  item;
...
int main(int argc, char** argv)
{
  ...
  parser.addOptionalParameter("b", "batchfile", cbica::Parameter::FILE, ".csv", "Input file with multipatient multimodality details", "Header format is as follows:'PATIENT_ID,", "T1,T2,FLAIR,T1CE,ROI,SELECTED_ROI,ROI_LABEL,", "SELECTED_FEATURES,PARAM_FILE'");
