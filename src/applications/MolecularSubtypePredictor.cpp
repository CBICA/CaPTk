 ...
 //   {
      ...
      for (size_t i = 0; i < results.size(); i++)
      {
        ...
        if (results[i] == 1)
          ...
          subtype = "Unknown";
        myfile << static_cast<std::string>(currentsubject[IMAGE_TYPE_SUDOID]) + "," + std::to_string(result_proneural[i]) + "," + std::to_string(result_neural[i]) + "," + std::to_string(result_messenchymal[i]) + "," +
