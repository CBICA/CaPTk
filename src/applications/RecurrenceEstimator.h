...
public:
	...
	*/
	void TrainNewModelOnGivenData(const std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects, const std::string &outputdirectory, bool useConventioalData, bool useDTIData, bool usePerfData, bool useDistData);

	/**
	\brief Recurrence Estimation using existing model
	*/
	void RecurrenceEstimateOnExistingModel(std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects,
	...
	const VectorVectorDouble &farRegionIndices)
{
	...
	//	else
	//	{
	mOutputLocalPtr.ReadModelParameters(
  ...
  if (ScaledTestingData.Cols() == 0)
  {
	...
	try
	{
		...
		result = DistanceFunction(ScaledTestingData, "../data/recurrence/" + mTrainedModelNameCSV, RECURRENCE_MODEL_RHO, RECURRENCE_MODEL_G);

    if (result.Size() == 0)
    {
      cbica::Logging(loggerFile, "{RecurrDebug}result.Size is empty");
    }
