...
std::vector<std::map<ImageModalityType, std::string>>  LoadQualifiedSubjectsFromGivenDirectoryForPseudoProgression(const MachineLearningApplicationSubtype type, const std::string &directoryname, const bool &useConventionalData, const bool &useDTIData, const bool &usePerfData, const bool &useDistData)
{
  ...
  for (unsigned int sid = 0; sid < subjectNames.size(); sid++)
  {
    ...
    OneQualifiedSubject[IMAGE_TYPE_FEATURES]  = featuresFilePath;
	...
	const std::string outputdirectory)
{
	...
	std::vector<std::map<ImageModalityType, std::string>> QualifiedSubjects = LoadQualifiedSubjectsFromGivenDirectoryForPseudoProgression(MachineLearningApplicationSubtype::TESTING, inputdirectory, true, true, true, true);
...
int PrepareNewSurvivalPredictionModel(const std::string inputdirectory,const std::string outputdirectory)
{
	...
	std::vector<double> finalresult;
  std::vector<std::map<ImageModalityType, std::string>> QualifiedSubjects = LoadQualifiedSubjectsFromGivenDirectoryForPseudoProgression(MachineLearningApplicationSubtype::TRAINING,inputdirectory,true,true,true,true);
	PseudoProgressionEstimator objPseudoProgressionEstimator;
	std::cout << "Number of subjects with required input: " << QualifiedSubjects.size() << std::endl;
...
int main(int argc, char **argv)
{
	...
	if (applicationType == MachineLearningApplicationSubtype::TESTING && modelDirectoryName=="")
	{
