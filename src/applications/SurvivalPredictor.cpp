...
int SurvivalPredictor::PrepareNewSurvivalPredictionModel(const std::string &inputdirectory,const std::vector< std::map< ImageModalityType, std::string > > &qualifiedsubjects, const std::string &outputdirectory)
{
...
//   //--------------------------------------read whole new data for training---------------------------------
   ...
   try
   {
	   trainOpenCVSVM(SixModelFeatures, outputdirectory + "/" + mSixTrainedFile, false, Survival);
	   trainOpenCVSVM(EighteenModelFeatures, outputdirectory + "/" + mEighteenTrainedFile, false, Survival);
   }
   catch (const std::exception& e1)
   {
...
VectorDouble SurvivalPredictor::SurvivalPredictionOnExistingModel(const std::string &modeldirectory, const std::string &inputdirectory, const std::vector < std::map < ImageModalityType, std::string>> &qualifiedsubjects, const std::string &outputdirectory)
{
	...
	for (unsigned int sid = 0; sid < qualifiedsubjects.size(); sid++)
	{
		...
		ImageType::Pointer LabelImagePointer = ReadNiftiImage<ImageType>(static_cast<std::string>(currentsubject[IMAGE_TYPE_SEG]));
		ImageType::Pointer AtlasImagePointer = ReadNiftiImage<ImageType>(static_cast<std::string>(currentsubject[IMAGE_TYPE_ATLAS]));
		ImageType::Pointer TemplateImagePointer = ReadNiftiImage<ImageType>(modeldirectory + "/Template.nii.gz");
		ImageType::Pointer RCBVImagePointer = RescaleImageIntensity<ImageType>(ReadNiftiImage<ImageType>(static_cast<std::string>(currentsubject[IMAGE_TYPE_RCBV])));
		ImageType::Pointer PHImagePointer = RescaleImageIntensity<ImageType>(ReadNiftiImage<ImageType>(static_cast<std::string>(currentsubject[IMAGE_TYPE_PH])));
		ImageType::Pointer T1CEImagePointer = RescaleImageIntensity<ImageType>(ReadNiftiImage<ImageType>(static_cast<std::string>(currentsubject[IMAGE_TYPE_T1CE])));
		ImageType::Pointer T2FlairImagePointer = RescaleImageIntensity<ImageType>(ReadNiftiImage<ImageType>(static_cast<std::string>(currentsubject[IMAGE_TYPE_T2FLAIR])));
	...
	try
	{
		...
		if (cbica::fileExists(modeldirectory + "/Survival_SVM_Model6.csv") == true && cbica::fileExists(modeldirectory + "/Survival_SVM_Model18.csv") == true)
		{
			...
			result_6 = DistanceFunction(SixModelSelectedFeatures, modeldirectory + "/Survival_SVM_Model6.csv", -1.0927, 0.0313);
			result_18 = DistanceFunction(EighteenModelSelectedFeatures, modeldirectory + "/Survival_SVM_Model18.csv", -0.2854, 0.5);
