  ...
  const VariableSizeMatrixType &configuration)
{
  ...
  {
    if (atlimIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR)
      etumorIndices.push_back(atlimIt.GetIndex());
    else if (atlimIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING)
      necoreIndices.push_back(atlimIt.GetIndex());
    else if (atlimIt.Get() == GLISTR_OUTPUT_LABELS::EDEMA)
      edemaIndices.push_back(atlimIt.GetIndex());
    else if (atlimIt.Get() == GLISTR_OUTPUT_LABELS::VENT)
      ventIndices.push_back(atlimIt.GetIndex());
    else
    {
    }
    if (atlimIt.Get() > GLISTR_OUTPUT_LABELS::ALL)
      brainIndices.push_back(atlimIt.GetIndex());
    if (atlimIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR || atlimIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING)
  ...
  {
    if (ncrAtlasIt.Get() == VOXEL_STATUS::ON)
      necoreIndices.push_back(ncrIt.GetIndex());
    ++ncrAtlasIt;
  }
  ...
  {
    if (etAtlasIt.Get() == VOXEL_STATUS::ON)
  ...
  {
    if (tumorAtlasIt.Get() == VOXEL_STATUS::ON)
  ...
  typename ImageType::Pointer  edemaDistanceMap = GetDistanceMapWithLabel<ImageType>(atlasImagePointer, GLISTR_OUTPUT_LABELS::EDEMA);
  ...
  {
    if (labelIt.Get() == ORIGINAL_ATLAS_LABELS::VENT1 || labelIt.Get() == ORIGINAL_ATLAS_LABELS::VENT2 || labelIt.Get() == ORIGINAL_ATLAS_LABELS::VENT3 || labelIt.Get() == ORIGINAL_ATLAS_LABELS::VENT4)
