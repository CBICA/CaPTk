...
void WhiteStripe::getHisInfo(vector<float>& mids, vector<float>& origHist, vector<float>& smoothHist, vector<int>& peakIds, int& modeId)
{
...
ImageType::Pointer WhiteStripe::process(ImageType::Pointer img, ImageType::Pointer& maskOut)
{
  ...
  try
  {
    ...
    vector<float>  whiteStripe, probs;
    ...
    vector<ImageType::IndexType> whiteStripeInd = Utils::getPixelIndicesThresholded(img, whiteStripe[0], whiteStripe[1]);
...
float WhiteStripe::threshMean(const vector<float>& voi, float thresh)
{
  double sum = 0;
...
vector<float> WhiteStripe::makeImageVoi(const ImageType::Pointer img)
{
...
double WhiteStripe::calcStdDev(const ImageType::Pointer& img, const vector<ImageType::IndexType>& indices)
{
...
ImageType::Pointer WhiteStripe::whitestripeIndToMask(ImageType::Pointer img, vector<ImageType::IndexType> indices)
{
  typedef itk::ImageDuplicator< ImageType > DuplicatorType;
  DuplicatorType::Pointer duplicator = DuplicatorType::New();
  duplicator->SetInputImage(img);
...
vector<ImageType::IndexType> WhiteStripe::whitestripeMasktoInd(ImageType::Pointer mask)
{
  vector<ImageType::IndexType> indices;
  typedef itk::ImageRegionConstIterator< ImageType > ConstIteratorType;
...
ImageType::Pointer WhiteStripe::whitestripeNorm(ImageType::Pointer img, vector<ImageType::IndexType> indices) {
  vector<float> pixelVals;
  for (size_t i = 0; i < indices.size(); i++)
  {
    pixelVals.push_back(img->GetPixel(indices[i]));
