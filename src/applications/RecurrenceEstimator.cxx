...
std::vector<std::map<ImageModalityType, std::string>> LoadQualifiedSubjectsFromGivenDirectoryForRecurrence(const MachineLearningApplicationSubtype type, const std::string &directoryname, const bool &useConventionalData, const bool &useDTIData, const bool &usePerfData, const bool &useDistData)
{
	...
	const std::string outputdirectory)
{
	...
	std::vector<std::map<ImageModalityType, std::string>> QualifiedSubjects = LoadQualifiedSubjectsFromGivenDirectoryForRecurrence(MachineLearningApplicationSubtype::TESTING,inputdirectory,true,true,true,true);
	...
	for (unsigned int subjectID = 0; subjectID < QualifiedSubjects.size(); subjectID++)
	{
		...
		std::cout << "Patient's data loaded: " << subjectID + 1 << "/" << QualifiedSubjects.size() << ", Sudo ID: " << static_cast<std::string>(OneQualifiedSubject[0][IMAGE_TYPE_SUDOID]) << std::endl;
...
int PrepareNewRecurrencePredictionModel(const std::string inputdirectory,const std::string outputdirectory)
{
	...
	std::vector<double> finalresult;
	std::vector<std::map<ImageModalityType, std::string>> QualifiedSubjects = LoadQualifiedSubjectsFromGivenDirectoryForRecurrence(MachineLearningApplicationSubtype::TRAINING,inputdirectory,true,true,true,true);
...
int main(int argc, char **argv)
{
	...
	//	}
	//}
	if (applicationType == MachineLearningApplicationSubtype::TESTING && modelDirectoryName.empty())
	{
	...
	if (applicationType == MachineLearningApplicationSubtype::TESTING)
	{
	...
	else if (applicationType == MachineLearningApplicationSubtype::TRAINING)
