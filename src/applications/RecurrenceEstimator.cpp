...
bool RecurrenceEstimator::TrainNewModelOnGivenData(const std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects, const std::string &outputdirectory, bool useConventionalData, bool useDTIData, bool usePerfData, bool useDistData)
{
	...
	for (unsigned int sid = 0; sid < qualifiedsubjects.size(); sid++)
	{
		...
		try
		{
			...
			{
				if (LabelImageIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR || LabelImageIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING)
				{
...
bool RecurrenceEstimator::RecurrenceEstimateOnExistingModel(std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects,
