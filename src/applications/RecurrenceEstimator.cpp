...
void RecurrenceEstimator::TrainNewModelOnGivenData(const std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects, const std::string &outputdirectory, bool useConventionalData, bool useDTIData, bool usePerfData, bool useDistData)
{
	...
	for (unsigned int sid = 0; sid < qualifiedsubjects.size(); sid++)
	{
		...
		ImageType::Pointer recurImagePointer = ReadNiftiImage<ImageType>(static_cast<std::string>(currentsubject[IMAGE_TYPE_NEAR]));
		ImageType::Pointer nonrecurImagePointer = ReadNiftiImage<ImageType>(static_cast<std::string>(currentsubject[IMAGE_TYPE_FAR]));
		...
		{
			if (LabelImageIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR_1 || LabelImageIt.Get() == GLISTR_OUTPUT_LABELS::TUMOR_2 || LabelImageIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_1 || LabelImageIt.Get() == GLISTR_OUTPUT_LABELS::NONENHANCING_2)
			{
		...
		mNiftiLocalPtr.LoadTrainingData(tumorMaskFinal, recurImagePointer, nonrecurImagePointer,
			T1CEImagePointer, T2FlairImagePointer, T1ImagePointer,
			T2ImagePointer, perfImagePointer, AXImagePointer,
			FAImagePointer, RADImagePointer, TRImagePointer,
			pNearIntensities, pFarIntensities, mNearIntensities,
			mFarIntensities, tNearIntensities, tFarIntensities, imagetype,
			useConventionalData, useDTIData, usePerfData, useDistData);
	...
	mOutputLocalPtr.SaveModelResults(ScaledTrainingData, mFeatureScalingLocalPtr.GetMeanVector(), mFeatureScalingLocalPtr.GetStdVector(), perfMeanVector, mFeatureReductionLocalPtr.GetPCATransformationMatrix(),
		useConventionalData, useDTIData, usePerfData, useDistData, size);
...
void RecurrenceEstimator::RecurrenceEstimateOnExistingModel(std::vector<std::map<ImageModalityType, std::string>> qualifiedsubjects,
	...
	bool useDTIData, bool usePerfData, bool useDistData)
{
	...
	for (unsigned int sid = 0; sid < qualifiedsubjects.size(); sid++)
	{
		...
		dilateFilter->SetDilateValue(GLISTR_OUTPUT_LABELS::EDEMA_1);
		dilateFilter->Update();
		dilatedEdema = dilateFilter->GetOutput();

		imagetype = NIfTI;
		testindices = mNiftiLocalPtr.LoadTestData(T1CEImagePointer, T2FlairImagePointer, T1ImagePointer, T2ImagePointer, perfImagePointer, AXImagePointer, FAImagePointer, RADImagePointer, TRImagePointer, LabelImagePointer, dilatedEdema, perfusionIntensities, otherIntensities, distanceIntensities, imagetype,
			useConventionalrData, useDTIData, usePerfData, useDistData);
		...
		while (!imIt.IsAtEnd())
		{
			if (!(imIt.Get() == GLISTR_OUTPUT_LABELS::EDEMA_1 || imIt.Get() == GLISTR_OUTPUT_LABELS::EDEMA_2))
		...
		if (imagetype == NIfTI)
			mOutputLocalPtr.WriteRecurrenceOutputInNifti<ImageType>(RecProbabilityMap, outputdirectory + "/" + static_cast<std::string>(currentsubject[IMAGE_TYPE_SUDOID]));
		else
			mOutputLocalPtr.WriteRecurrenceOutputInNifti<ImageType>(RecProbabilityMap, outputdirectory + "/" + static_cast<std::string>(currentsubject[IMAGE_TYPE_SUDOID]));
	}
	mFeatureReductionLocalPtr.ResetParameters();
