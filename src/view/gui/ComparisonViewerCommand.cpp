...
ComparisonViewerCommand::ComparisonViewerCommand()
{
  mStartSlicer = -1;
  mSlicerNumber = -1;
  mw = NULL;
...
int ComparisonViewerCommand::FindSlicerNumber(vtkRenderWindow* renwin)
{
  int rvalue;
...
void ComparisonViewerCommand::Execute(vtkObject *caller, unsigned long event, void *vtkNotUsed(callData))
{
  ...
  if (isi)
  {
    ...
    //if (VisibleInWindow >= 0)
    {
      ...
      {
        mStartSlicer = -1;
        this->InitialWindow = this->m_currentViewer->GetColorWindow();
        this->InitialLevel = this->m_currentViewer->GetColorLevel();
        return;
      }
    ...
    if (event == vtkCommand::WindowLevelEvent /*&& mStartSlicer >= 0 && this->SM->GetPreset() <= 1*/)
    {
      ...
      for (int i = 0; i < this->m_ComparisonViewers.size(); i++)
      {
        this->m_ComparisonViewers[i]->SetColorWindow(window*dx);
        this->m_ComparisonViewers[i]->SetColorLevel(level - dy);
        this->m_ComparisonViewers[i]->Render();
      }
      mw->SetWindowLevel((window*dx), (level - dy));
      return;
    }
  }
}
void ComparisonViewerCommand::AddActions()
{
}

void ComparisonViewerCommand::Dolly(double factor, vtkRenderWindowInteractor *interactor)
{
...
PointVal ComparisonViewerCommand::point2Dto3D(const std::pair<int, int >& pt, const int orientation, const int slice, const int value)
{
  PointVal pt3D;
  if (orientation == 2)//SLICE_ORIENTATION_XY
  {
...
std::vector< std::pair<int, int> > ComparisonViewerCommand::points3Dto2D(const std::vector<PointVal>& points3D, const int orientation)
{
...
std::vector<PointVal> ComparisonViewerCommand::points2Dto3D(const std::vector< std::pair<int, int> >& points2D, const int orientation, const int slice, const int value)
{
...
std::vector< std::pair<int, int> > ComparisonViewerCommand::getCirclePoints(int x, int y, int r)
{
  if (r <= 0) r = 1;
...
std::vector< std::pair<int, int> > ComparisonViewerCommand::getLinePoints(int x0, int y0, int x1, int y1, float wd)
{
  std::vector< std::pair<int, int> > points;
...
std::vector<PointVal> ComparisonViewerCommand::drawLine(PointVal startPt, PointVal endPt, vtkSmartPointer<vtkImageData> image, const int orientation, const int slice, int width)
{
...
std::vector<PointVal> ComparisonViewerCommand::fillShape(PointVal centerPt, int labelToFill, vtkSmartPointer<vtkImageData> image, const int orientation, const int slice)
{
  std::vector<PointVal> undoBuffer;
...
std::vector<PointVal> ComparisonViewerCommand::drawRectangle(PointVal startPt, PointVal endPt, vtkSmartPointer<vtkImageData> image, const int orientation, const int slice, int thickness)
{
...
std::vector<PointVal> ComparisonViewerCommand::drawSphere(PointVal startPt, vtkSmartPointer<vtkImageData> image, int radius)
{
  std::vector<PointVal> undoBuffer;
...
std::vector<PointVal> ComparisonViewerCommand::drawCircle(PointVal startPt, PointVal endPt, vtkSmartPointer<vtkImageData> image, const int orientation, const int slice, int width)
{
  std::vector<PointVal> undoBuffer;
  std::pair<int, int> p1 = point3Dto2D(startPt, orientation);
  std::pair<int, int> p2 = point3Dto2D(endPt, orientation);
...
PointVal ComparisonViewerCommand::drawPoint(PointVal pt, vtkSmartPointer<vtkImageData> image)
{
  if (image == NULL)
  {
...
PointVal ComparisonViewerCommand::drawPoint(PointVal pt, vtkSmartPointer<vtkImageData> image, const int orientation, const int slice, const int i, const int j)
{
  if (image == NULL)
  {
    return pt.getInvalidPt();
