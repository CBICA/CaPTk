...
void fMolecularSubtypePredictor::ConfirmButtonPressed()
{
  if (rdExistingClassification->isChecked())
  {
    ...
    //}
	//--------------function call--------------------
    emit MolecularSubtypePredictionOnExistingModel(svmModelFileName->text().toStdString(), testSubjectsDirectoryName->text().toStdString(), outputDirectoryName->text().toStdString());
  }
  else 
  {
    ...
    //}
	//--------------function call--------------------
    emit PrepareNewMolecularSubtypePredictionModel(existingMaskDirectoryName->text().toStdString(), outputDirectoryName->text().toStdString());
  }
  this->close();
}
...
void fMolecularSubtypePredictor::CheckForDisclaimer()
{
  ...
  if (box->exec() == QMessageBox::Ok)
  {
    ...
    std::string link = 
      ...
      "wget"
#endif
      + std::string(" ftp://www.nitrc.org/home/groups/captk/downloads/SampleData_1.6.0/MolecularSubtypePredictor_PretrainedModel.zip") +
      " -O " + captk_PretrainedFolder + "/MolecularSubtypePredictor_PretrainedModel.zip";
    ...
    if /*(std::system((link).c_str()) != 0)*/ (!openLink("ftp://www.nitrc.org/home/groups/captk/downloads/SampleData_1.6.0/MolecularSubtypePredictor_PretrainedModel.zip"))
    {
