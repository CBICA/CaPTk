...
namespace cbica
{
  ...
  */
  ...
  Type-casted to double for maximum accuracy
  */
  inline double GetDistanceBetweenIndeces(const float* point1, const float* point2)
  {
    double currentDist = 0.0;
    for (size_t i = 0; i < 3; i++)
    {
      currentDist += static_cast<double>(powf(point1[i] - point2[i], 2));
    }
    currentDist = static_cast<double>(sqrtf(currentDist));

    return currentDist;
  }

  /**
  \brief Get the distance between 2 float vectors

  Type-casted to double for maximum accuracy
  */
  template< class TDataType = double >
  inline double GetDistanceBetweenIndeces(const std::vector< TDataType >& point1, const std::vector< TDataType >& point2)
  {
    double currentDist = 0.0;
    if (point1.size() == point2.size())
    {
      ...
      {
        double temp = static_cast<double>(point1[i] - point2[i]);
        currentDist += temp * temp;
      }
      currentDist = static_cast<double>(sqrtf(currentDist));
    }
    return currentDist;
  ...
  \return The maximum distance and the corresponding index
  */
  template< class TImageType = ImageTypeFloat3D >
  ...
  /**
  \brief Create a blank image from an input
  */
  template< class TImageType >
  typename TImageType::Pointer CreateITKImage(typename TImageType::Pointer originalimage)
  {
    typename TImageType::Pointer new_image = TImageType::New();
    new_image->SetRegions(originalimage->GetLargestPossibleRegion());
    //new_image->SetRequestedRegion(originalimage->GetRequestedRegion());
    //new_image->SetBufferedRegion(originalimage->GetBufferedRegion());
    new_image->Allocate();
    new_image->FillBuffer(0);
    new_image->SetDirection(originalimage->GetDirection());
    new_image->SetOrigin(originalimage->GetOrigin());
    new_image->SetSpacing(originalimage->GetSpacing());
    return new_image;
  }
