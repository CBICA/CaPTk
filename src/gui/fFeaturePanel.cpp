...
void fFeaturePanel::featureTypeChanged(int type)
{
  ...
  if (!currentMap.empty())
  {
    for (auto ckh : m_featureCheckBoxMap)
    {
      string name = ckh.first;
      if (currentMap[name].empty())
      {
        ckh.second->setChecked(false);
        ckh.second->setEnabled(false);
      }
      else
      {
        //if (FeatureExtraction::Custom == type)
        //{
        //  ckh.second->setEnabled(true);
        //}
        //else
        //{
        //  ckh.second->setEnabled(false);
        //}
        ckh.second->setChecked(true);
      }
    }
  }
