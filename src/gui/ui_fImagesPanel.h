...
public:
  ...
  void setupUi(QWidget *fImagesPanel)
  {
    ...
    for (int i = 0; i < m_nonVisImagesTable->model()->columnCount(); i++)
    {
      if (i == nameColID)
      {
        m_nonVisImagesTable->horizontalHeader()->setResizeMode(i, QHeaderView::Stretch);
      }
      else
      {
        ...
        m_nonVisImagesTable->horizontalHeader()->setResizeMode(i, QHeaderView::Fixed);
    ...
    m_overlayChkBox = new QCheckBox("Enable overlay");
