...
public:
  ...
  */
  ...
  void FormulateNearFarPoints(std::vector<typename ImageType::IndexType> &nearIndices, std::vector<typename ImageType::IndexType> &farIndices)
  {
    ...
    itk::ImageRegionIteratorWithIndex <ImageType> maskIt(img, img->GetLargestPossibleRegion());
    maskIt.GoToBegin();
    while (!maskIt.IsAtEnd())
    {
      if (maskIt.Get() == 1)
...
signals:
  ...
  */
  void CallDCM2NIfTIConversion(const std::string firstImageInSeries);
  ...
  */
  void help_Download(QAction* action)
  {
    ...
    //  std::string link =path + "/../../data/GnuWin32/bin/wget.exe ftp://www.nitrc.org/home/groups/captk/downloads/SampleData_1.5.0/" + action->text().toStdString() + ".zip" +
    //	  " -O " + tempFolderLocation + "/" + action->text().toStdString() + ".zip";

    ////std::string link2 = path + "E:/SoftwareDevelopmentProjects/captk-dev-02-22-2018/data/GnuWin32/bin/wget.exe ftp://www.nitrc.org/home/groups/captk/downloads/SampleData_1.5.0/" + action->text().toStdString() + ".zip";
    ////#if WIN32
    ////    command = "start ";
    ////#elif __linux__
    ////    command = "xdg-open ";
    ...
    //else
    //{
    //	std::string dataMessage = "Data has been saved to: " + tempFolderLocation;
    //	ShowErrorMessage(dataMessage);
    //	return;
    //}
    ...
    std::string link =
#ifdef _WIN32
      path + "data/GnuWin32/bin/wget.exe"
#else
      "wget"
#endif
      + std::string(" ftp://www.nitrc.org/home/groups/captk/downloads/SampleData_1.5.0/") + currentApp + ".zip" +
  ...
  */
  ...
  void ApplicationSBRTSegment();
  void ApplicationSBRTAnalyze();
  ...
  */
  bool PreparePythonScriptAndConfig(const std::string &NameToCheck, const std::string &AppName, std::string &scriptWithPath);

  /**
  \brief Search for the Python script and its matching the config file in many locations WRT to the running graphical layer executable
  */
  bool PreparePythonScriptAndConfig(const std::string &NameToCheck, const std::string &AppName, std::string &scriptWithPath, std::string &configFileWithPath);
