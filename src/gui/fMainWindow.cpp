...
#else
  std::string args_string = "", app_string = application.toStdString();
...
fMainWindow::fMainWindow()
{
  ...
  for (size_t i = 0; i < vectorOfLungApps.size(); i++)
  {
    ...
    {
      vectorOfLungApps[i].action->setText("  Radiomics Analysis of Lung Cancer (SBRT Lung) Segment"); //TBD set at source
      connect(vectorOfLungApps[i].action, SIGNAL(triggered()), this, SLOT(ApplicationSBRTSegment()));
    }
    //else if (vectorOfLungApps[i].name.find("SBRT_Analyze") != std::string::npos)
    //{
    //  vectorOfLungApps[i].action->setText("  Radiomics Analysis of Lung Cancer (SBRT Lung) Analyze"); //TBD set at source
    //  connect(vectorOfLungApps[i].action, SIGNAL(triggered()), this, SLOT(ApplicationSBRTAnalyze()));
    //}
  }
  ...
  for (size_t i = 0; i < vectorOfMiscApps.size(); i++)
  {
    ...
    else if (vectorOfMiscApps[i].name.find("DirectionalityEstimate") != std::string::npos)
    {
    ...
    //{
    //  vectorOfMiscApps[i].action->setText("  DeepMedic Segmentation"); // TBD set at source
    //  connect(vectorOfMiscApps[i].action, SIGNAL(triggered()), this, SLOT(DeepMedicSegmentation()));
    //}
  }
  ...
  recurrencePanel.SetCurrentLoggerPath(tempFolderLocation);
  msubtypePanel.SetCurrentLoggerPath(tempFolderLocation);
  survivalPanel.SetCurrentLoggerPath(tempFolderLocation);
...
void fMainWindow::LoadedSubjectExistingRecurrenceEstimate(const std::string &outputdirectory, const std::string &modeldirectory, bool convDataPresent, bool dtiDataPresent, bool perfusionDataPresent, bool distanceDataPresent)
{
  ...
  labels.push_back(GLISTR_OUTPUT_LABELS::TUMOR);
  labels.push_back(GLISTR_OUTPUT_LABELS::NONENHANCING);
...
void fMainWindow::CallGeneratePopualtionAtlas(const std::string inputdirectory, const std::string inputlabel, const std::string inputatlas, const std::string outputdirectory)
{
  ...
  if (mPopulationAtlas.mLastErrorMessage.empty() && atlases.size() > 0)
  {
    ...
    LoadSlicerImages(inputatlas, NIfTI);
...
void fMainWindow::LoadDrawing()
{
  if (!mSlicerManagers.empty())
  {
    ...
    dialog.setDirectory(mInputPathName);
    dialog.setFileMode(QFileDialog::ExistingFile);
    dialog.selectFile(extensions);
...
#ifndef _WIN32
  scriptToCall = QApplication::applicationDirPath().toStdString() + "/ConfettiGUI.py";
#endif

  //if (std::system(scriptToCall.c_str()) != 0)
  //{
...
void fMainWindow::ApplicationSBRTSegment()
{
  ...
  std::string ctImageFile, petImageFile, command, exePath,
    maskFile = tempFolderLocation + "/tempMask.nii.gz";
  for (unsigned int index = 0; index < mSlicerManagers.size(); index++)
  {
    if (mSlicerManagers[index]->mImageSubType == IMAGE_TYPE_CT)
    {
      ctImageFile = mSlicerManagers[index]->mPathFileName;
    }
    else if (mSlicerManagers[index]->mImageSubType == IMAGE_TYPE_PET)
    {
  ...
  QStringList args;
  args << "-c" << ctImageFile.c_str() << "-p" << petImageFile.c_str() << "-m" << maskFile.c_str() << "-o" << tempFolderLocation.c_str();
  startExternalProcess(command.c_str(), args);

  //if (std::system((scriptToCall + "-c " + ctImageFile + "-p " + petImageFile + "-m " + maskFile).c_str()) != 0)
  //{
  //  ShowErrorMessage("SBRT Segmentation did not exit successfully; please see log file for details: " + loggerFile);
  //}
  ...
  {
    ShowErrorMessage("Error in SBRT Segmentation", this);
  }

}
void fMainWindow::ApplicationSBRTAnalyze()
{
  if (mSlicerManagers.size() < 2)
  {
    ShowErrorMessage("Load two images with first being the CT and second being the PET image.", this);
  ...
  QMessageBox *box = new QMessageBox(QMessageBox::Question, "SBRT Analysis Pre-trained", "The model file was trained using data from UPenn (see documentation for details) and the results will be based on that and by proceeding, you acknowledge this.", QMessageBox::Ok | QMessageBox::Cancel);
  ...
  if (box->exec() == QMessageBox::Ok)
  {
    ...
    //if (std::system((scriptToCall + "-c " + ctImageFile + "-p " + petImageFile + "-m " + maskFile + "-t " + modelfilename + "-o " + outputFile).c_str()) != 0)
    //{
    //  ShowErrorMessage("SBRT Analyze did not exit successfully; please see log file for details: " + loggerFile);
    //}
    ...
    if (cbica::fileExists(outputFile))
    {
      ...
      if (infile.is_open())
      {
        infile >> result;
        if (result > 0.5)
          ShowMessage("Prediction Result: " + std::to_string(result) + "\n\nNo signs of nodal failure");
        else                                                            
          ShowMessage("Prediction Result: " + std::to_string(result) + "\n\nSigns of nodal failure");
      }
      else
      {
        ShowErrorMessage("SBRT Analyze couldn't process the images", this);
      }
    }
    else
    {
      ShowErrorMessage("SBRT Analyze failed to produce a result. Please provide an ROI", this);
    }
...
void fMainWindow::ApplicationRecurrence()
{
  ...
  //if (fileNames.size())
  {
    recurrencePanel.SetCurrentImagePath(tempFolderLocation.c_str());
...
void fMainWindow::ApplicationITKSNAP()
{
  ...
  std::string scriptToCall = QApplication::applicationDirPath().toStdString() +
#ifdef Q_OS_WIN32
    "/itksnap.exe"
#else
    "/itksnap"
  ...
  //  std::system((scriptToCall + "-w " + tempFolderLocation + "/testXML.itksnap").c_str());
  readMaskFile(maskFile);
...
//{
//  const std::string errorMsg = "DeepMedic needs the following images to work: T1CE, T1, T2, FLAIR";
//  if (mSlicerManagers.size() < 4)
//  {
//    ShowErrorMessage(errorMsg, this);
//    return;
//  }
//
...
//  }
//
//  std::string dataDir = deepMedicDialog.m_dataDir.toStdString() + "deepMedic/";
//
//  QStringList args;
...
//
//  ShowMessage("DeepMedic setup:\nExeName  :" + deepMedicDialog.m_exe.toStdString() +
//    "\nDataDir  : " + dataDir +
//    "\nCommand  : " + args.join(" ").toStdString() +
//    "\nFile_T1CE: " + file_t1ce + "\nFile_T1  : " + file_t1 + "\nFile_T2  : " + file_t2 + "File_FL  : " + file_flair);
//
//  if (!cbica::exists(deepMedicDialog.m_exe.toStdString()))
//  {
//    ShowErrorMessage("DeepMedic executable doesn't exist; can't run");
//    return;
//  }
//
//  if (startExternalProcess(deepMedicDialog.m_exe, args) != 0)
//  {
  ...
  const std::string inputImageFile, const std::string outputImageFile)
{
  ...
  if ((cbica::getFilenameExtension(outputImageFile) != ".nii") && (cbica::getFilenameExtension(outputImageFile) != ".nii.gz"))
  {
    ...
    LoadSlicerImages(path + base + ".nii.gz", NIfTI);
  }
  else
  {
...
void fMainWindow::CallPerfusionMeasuresCalculation(const double TE, const bool rcbv, const bool  psr, const bool ph, const std::string inputfilename, std::string outputFolder)
{
  ...
  std::vector<typename ImageType::Pointer> perfusionDerivatives = m_perfusionderivatives.Run<ImageTypeFloat3D, ImageTypeFloat4D>(inputfilename, rcbv, psr, ph, TE);
...
void fMainWindow::CallPCACalculation(const int number, const std::string outputFolder)
{
  ...
  ImageType::Pointer maskImage = convertVtkToItk<float, 3>(mSlicerManagers[0]->mMask);
